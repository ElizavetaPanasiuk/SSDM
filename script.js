...
getCompareFieldsBut.addEventListener('click', function func() {
  let lis = document.querySelectorAll('.listItem').forEach(elem => {
  ...
  for (let i = 0; i < varCount; i++){
    for (let j = i + 1; j < varCount; j++){
      ...
      choiceDiv.innerHTML = `<p class="q"><b>${variants[i].variant}</b> or <b>${variants[j].variant}</b>?</p>`;
      ...
      p1.append(var1);
      p1.innerHTML += variants[i].variant;
      ...
      p2.append(var2);
      p2.innerHTML += variants[j].variant;
...
resultBut.addEventListener('click', function func() {
  ...
  resultField.innerHTML += `The best choice is <b>${bestVar}</b> with score <b>${bestVarScore}/${variants.length - 1}</b>`;
  ...
  for (let i = 0; i < variants.length; i++){
    if (variants[i].variant !== bestVar) {
      resultField.innerHTML += `<p>The score of variant <b>${variants[i].variant}</b> is <b>${variants[i].score}/${variants.length - 1}</b></p>`
    }
  }
